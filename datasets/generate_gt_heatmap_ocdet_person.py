"""
Usage:
    python datasets/generate_gt_heatmap_ocdet_person.py [options]

Description:
    (deprecated) use generate_gt_heatmap_ocdet_person.py instead
    This script processes COCO dataset images to generate centerness masks for person class only, using either single- or multiprocessing depending on the command-line arguments.
...
Examples:
    ...
    python generate_gt_heatmap_ocdet_person.py -m --use_seg

    # To generate centerness masks for the training set using a multiprocessing:
    python generate_gt_heatmap_ocdet_person.py --train --multiprocessing --use_seg
"""
...
def generate_map_one_image(image, target, mask_obj_func, use_seg, coco):
    ...
    for obj in target:
        if obj["iscrowd"] or obj["category_id"] != 1:
            continue
...
def generate_maps_coco(
    ...
    mask_folder = os.path.join(coco_folder, mask_folder_name, "1", split)  # 1:person
