"""
Usage:
    python datasets/generate_gt_heatmap_ocdet_one_class.py [options]

Description:
    This script processes COCO dataset images to generate centerness masks for one specified class, using either single- or multiprocessing depending on the command-line arguments.
...
Examples:
    ...
    python datasets/generate_gt_heatmap_ocdet_one_class.py -m --use_seg

    # To generate centerness masks for the training set using a multiprocessing:
    python datasets/generate_gt_heatmap_ocdet_one_class.py --train --multiprocessing --use_seg

    # To generate centerness masks for the validation set using a single process with iregular shape and only for category 44:
    python datasets/generate_gt_heatmap_ocdet_one_class.py -i --id 44
"""
...
ETAs = [0.7, 0.1, 0.3, 0.7, 0.9]
PHIs = [0.7, 0.9, 0.7, 0.3, 0.1]
...
def get_generalized_centerness(h, w, eta=0.5, phi=0.5):
    """
    given a bounding box of width w and height h, returns a weighted centerness score
    default values 0.5 for both eta and phi gives the same result as centerness
    """
    centerness = np.zeros((h, w))
    # for i in range(w):
    #     for j in range(h):
    ...
    #         t = j + 0.5
    #         b = h - j - 0.5
    #         centerness[j][i] = (np.minimum(l, r) / np.maximum(l, r)) ** eta * (
    ...
    centerness = (np.minimum(l, r) / np.maximum(l, r)) ** eta * (
        np.minimum(t, b) / np.maximum(t, b)
    ) ** phi
...
def generate_map_one_image(image, target, mask_obj_func, use_seg, coco, category_id=1):
    """
    ...
    for obj in target:
        if obj["iscrowd"] or obj["category_id"] != category_id:
            continue
        ...
        mask_img[y_slice, x_slice] = np.maximum(
            mask_img[y_slice, x_slice],
            mask_obj,
        )
...
def generate_map_one_image_resize(
    image, target, mask_obj_func, use_seg, coco, resized_shape, category_id=1
):
    """
    Generate a mask for a single image based on the given target objects.
    Args:
    ...
    mask_img = np.zeros((H_RESIZED, W_RESIZED))
    ...
    for obj in target:
        ...
        if bw_resized < 1 or bh_resized < 1:
            ...
            mask_img[by_resized, bx_resized] = 1
            continue
...
def generate_maps_coco(
    ...
    if mask_type == "centerness":
        mask_obj_func = partial(get_generalized_centerness, eta=eta, phi=phi)
    ...
    mask_folder = os.path.join(coco_folder, mask_folder_name, str(category_id), split)
    os.makedirs(mask_folder, exist_ok=True)

    # Load COCO dataset
    coco = COCO(annFile)
    ids = coco.getImgIds(catIds=[category_id])
...
def process_image(
    ...
    mask_path = os.path.join(mask_folder, f"{file_name.split('.')[0]}.png")
    ...
    if resized_shape:
        mask_img = generate_map_one_image_resize(
            image, target, mask_obj_func, use_seg, coco, resized_shape, category_id
        )
    else:
        mask_img = generate_map_one_image(
            image, target, mask_obj_func, use_seg, coco, category_id
        )

    # Save the mask if not empty
    if np.all(mask_img == 0):
        return

    mask = Image.fromarray((mask_img * 255).astype(np.uint8))
    mask.save(mask_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--irregular_shape", "-i", action="store_true")
    parser.add_argument("--id", default=1, type=int, help="category id")
